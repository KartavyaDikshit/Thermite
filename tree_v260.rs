...
impl DecisionTreeClassifier {
    ...
    fn find_best_classification_split(
        ...
        parent_counts: &[f64],
        total: f64,
    ) -> Option<(usize, f64, bool, Vec<f64>, Vec<usize>, Vec<usize>, bool)> {
        ...
        for &feat in candidate_features {
            if self.categorical_features.contains(&feat) {
                ...
                let mut cats: Vec<(f64, f64, f64, Vec<f64>)> = cat_counts
                    .into_iter()
                    .map(|(k, v)| {
                ...
                for i in 0..cats.len() - 1 {
                    ...
                    let weighted =
                        (left_total * left_impurity + right_total * right_impurity) / total;
                ...
                let (nan_indices, non_nan_indices): (Vec<usize>, Vec<usize>) = indices
                    .iter()
                    .cloned()
                    .partition(|&idx| X[[idx, feat]].is_nan());
                if non_nan_indices.len() < 2 {
                    continue;
                }
                let sorted = sorted_indices_by_feature(X, &non_nan_indices, feat);
                ...
                for i in 0..sorted.len() - 1 {
                    ...
                    // Scenario 1: Route all NaNs to the left child
                    let s1_left_total = left_non_nan_total + nan_total;
                    let s1_right_total = right_non_nan_total;
                    if (s1_left_total as usize) >= self.min_samples_leaf
                        && (s1_right_total as usize) >= self.min_samples_leaf
                    {
                        let mut s1_left_counts = left_non_nan_counts.clone();
                        for c in 0..self.n_classes {
                            s1_left_counts[c] += nan_counts[c];
                        }
                        let left_impurity = gini_impurity(&s1_left_counts, s1_left_total);
                        let right_impurity = gini_impurity(&right_non_nan_counts, s1_right_total);
                        let weighted = (s1_left_total * left_impurity
                            + s1_right_total * right_impurity)
                            / total;
                        let gain = parent_impurity - weighted;
                        if gain > best_gain {
                            best_gain = gain;
                    ...
                    // Scenario 2: Route all NaNs to the right child
                    let s2_left_total = left_non_nan_total;
                    let s2_right_total = right_non_nan_total + nan_total;
                    if (s2_left_total as usize) >= self.min_samples_leaf
                        && (s2_right_total as usize) >= self.min_samples_leaf
                    {
                        let mut s2_right_counts = right_non_nan_counts.clone();
                        for c in 0..self.n_classes {
                            s2_right_counts[c] += nan_counts[c];
                        }
                        let left_impurity = gini_impurity(&left_non_nan_counts, s2_left_total);
                        let right_impurity = gini_impurity(&s2_right_counts, s2_right_total);
                        let weighted = (s2_left_total * left_impurity
                            + s2_right_total * right_impurity)
                            / total;
                        let gain = parent_impurity - weighted;
                        if gain > best_gain {
                            best_gain = gain;
    ...
    fn predict_proba_single(&self, X: &ArrayView2<f64>, sample_idx: usize) -> &[f64] {
        ...
        loop {
            ...
            if val.is_nan() {
                ...
                if node.left_categories.contains(&val) {
                    node_idx = node.left;
                } else {
                    node_idx = node.right;
                }
    ...
    fn find_best_regression_split(
        ...
        for &feat in candidate_features {
            if self.categorical_features.contains(&feat) {
                ...
                let mut cats: Vec<(f64, f64, f64, f64, f64)> = cat_stats
                    .into_iter()
                    .map(|(k, v)| {
                        let val = f64::from_bits(k);
                        let mean = v.1 / v.0;
                ...
                let mut nan_sum = 0.0;
                let mut nan_sum_sq = 0.0;
                let mut nan_count = 0.0;
                for &idx in &nan_indices {
                    let val_y = y[idx];
                    nan_sum += val_y;
                    nan_sum_sq += val_y * val_y;
                    nan_count += 1.0;
                }
                ...
                let mut right_non_nan_sum = 0.0;
                let mut right_non_nan_sum_sq = 0.0;
                let mut right_non_nan_count = 0.0;
                for &idx in &sorted {
                    let val_y = y[idx];
                    right_non_nan_sum += val_y;
                    right_non_nan_sum_sq += val_y * val_y;
                    right_non_nan_count += 1.0;
                }

                for i in 0..sorted.len() - 1 {
                    let idx = sorted[i];
                    let val_y = y[idx];
                    left_non_nan_sum += val_y;
                    left_non_nan_sum_sq += val_y * val_y;
                    left_non_nan_count += 1.0;
                    ...
                    let s1_right_sum = right_non_nan_sum;
                    let s1_right_sum_sq = right_non_nan_sum_sq;
                    let s1_right_count = right_non_nan_count;

                    if (s1_left_count as usize) >= self.min_samples_leaf
                        && (s1_right_count as usize) >= self.min_samples_leaf
                    {
                        let left_mse = mse_from_stats(s1_left_sum, s1_left_sum_sq, s1_left_count);
                        let right_mse =
                            mse_from_stats(s1_right_sum, s1_right_sum_sq, s1_right_count);
                        let weighted =
                            (s1_left_count * left_mse + s1_right_count * right_mse) / total_count;
                        let gain = parent_mse - weighted;
                        if gain > best_gain {
                            best_gain = gain;
                            let threshold = (val + next_val) / 2.0;
                    ...
                    // Scenario 2: Route all NaNs to the right child
                    let s2_left_sum = left_non_nan_sum;
                    let s2_left_sum_sq = left_non_nan_sum_sq;
                    ...
                    if (s2_left_count as usize) >= self.min_samples_leaf
                        && (s2_right_count as usize) >= self.min_samples_leaf
                    {
                        let left_mse = mse_from_stats(s2_left_sum, s2_left_sum_sq, s2_left_count);
                        let right_mse =
                            mse_from_stats(s2_right_sum, s2_right_sum_sq, s2_right_count);
                        let weighted =
                            (s2_left_count * left_mse + s2_right_count * right_mse) / total_count;
                        let gain = parent_mse - weighted;
                        if gain > best_gain {
                            best_gain = gain;
                            let threshold = (val + next_val) / 2.0;
...
mod tests {
    ...
    fn test_classifier_min_samples_leaf() {
        ...
        let leaf_counts: Vec<usize> = tree
            .nodes
            .iter()
            .filter(|n| n.is_leaf())
            .map(|n| n.n_samples)
    ...
    #[test]
    fn test_classifier_nan_split() {
        let X = array![[1.0], [2.0], [f64::NAN], [f64::NAN], [9.0], [10.0]];
        let y = vec![0.0, 0.0, 0.0, 1.0, 1.0, 1.0];
        let mut tree = DecisionTreeClassifier::new(None, 2, 1, None, Some(42));
        tree.fit(&X.view(), &y);
    ...
    #[test]
    fn test_regressor_nan_split() {
        let X = array![[1.0], [2.0], [f64::NAN], [f64::NAN], [9.0], [10.0]];
        let y = vec![1.0, 2.0, 1.5, 9.5, 9.0, 10.0];
        let mut tree = DecisionTreeRegressor::new(None, 2, 1, None, Some(42));
        tree.fit(&X.view(), &y);
